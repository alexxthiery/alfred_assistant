...
| **Email review** (10:00) | **Yes** — triage + judgment | `run-email-review.sh`: invokes a headless Alfred from the vault, points it at `AGENTS.md` + `persona/email-review.md`, runs `.bin/email-review --days 1 --max-questions 7 --record-ledger`, and sends only concrete questions or useful updates to Telegram. |
...
5. **Policy files are explicit:** if a headless agent runs, the wrapper prompt must point it at the relevant deployed instructions. Email review reads `AGENTS.md` plus `persona/email-review.md`.
