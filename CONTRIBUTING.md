...
# 2. Install the hooks (pre-commit: PII scan + test suite; pre-push: PII scan
# of every pushed commit)
ln -sfn ../../tools/pre-commit .git/hooks/pre-commit
ln -sfn ../../tools/pre-push .git/hooks/pre-push
# If a global core.hooksPath shadows the per-repo hook, override:
git config --local core.hooksPath .git/hooks

# 3. Point the PII guard at your vaults (gitignored), one path per line, then
# build the pattern list. The hooks rebuild it on every commit and push.
# Optional: tools/pii-extra.local.txt (hand-kept <CATEGORY>\t<pattern> lines)
# and tools/pii-allow.local.txt (public names to ignore).
$EDITOR tools/pii-vaults.local.txt
tools/build-pii-list.js
npm run doctor
...
- `tools/pii-list.local.txt` — your local PII patterns (generated by `tools/build-pii-list.js`)
- `tools/pii-vaults.local.txt`, `tools/pii-extra.local.txt`, `tools/pii-allow.local.txt` — PII guard inputs
