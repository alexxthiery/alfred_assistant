...
3. **Keep secrets vault-private and out of plists.** LaunchAgents should only contain paths, scheduling metadata, and environment-variable pointers such as `ALFRED_VAULT`, `ALFRED_ASSISTANT_LABEL`, `ENV_FILE`, and `PATH`. `ENV_FILE` must be inside the active vault's `.alfred/private/` directory, must not be a symlink, and must declare matching `ALFRED_EXPECTED_VAULT` and `ALFRED_EXPECTED_LABEL`. Wrappers must fail before sourcing an env file that violates this.
