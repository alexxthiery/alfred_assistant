...
When {{USER_NAME}} asks for proactive email analysis interactively, run `/workspace/extra/vault/.bin/email-review --days <D>` where `<D>` is the requested window, defaulting to 1 for a daily review. Use `--max-questions 7` for daily-style triage so the question cap is explicit. Record the ledger only after the report has actually been surfaced or handled; for JSON reports use `--record-from-json`.

For the scheduled 10:00 run, `integrations/scheduling/run-email-review.sh` runs `.bin/email-review --format json` directly and then passes the metadata-only report to the headless agent. If a scheduler prompt already contains an `EMAIL REVIEW JSON REPORT`, do not run `.bin/email-review`, `.bin/gmail`, or any other Gmail command again; use the provided report as the Gmail evidence for that pass.
