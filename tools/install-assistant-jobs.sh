...
#
# The host wrappers (run-daily-brief.sh, run-reminder-dispatch.sh,
# vault-backup-push.sh) are already generic: each plist supplies ALFRED_VAULT +
...
#       [--brief HH:MM] [--reminders] [--backup HH:MM] [--wrappers <dir>] \
#       [--dry-run] [--uninstall]
#
#   --vault       the assistant's vault (contains .bin/)
#   --env         env file the wrappers source (EMAIL_FROM / GMAIL_APP_PASSWORD /
#                 TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID — set whichever apply)
#   --label       short slug; plists are com.<label>.{daily-brief,reminder-dispatch,vault-backup-push}
...
#
# At least one of --brief / --reminders / --backup is required (unless --uninstall).
set -euo pipefail

VAULT="" ENVFILE="" LABEL="" WRAPPERS="$HOME/.local/bin"
BRIEF="" REMINDERS=0 BACKUP="" DRYRUN=0 UNINSTALL=0
PATH_LINE="/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin"
...
if [ "$UNINSTALL" -eq 1 ]; then
  for kind in daily-brief reminder-dispatch vault-backup-push; do
...
[ -n "$BRIEF$BACKUP" ] || [ "$REMINDERS" -eq 1 ] || { echo "install-assistant-jobs: pick at least one of --brief / --reminders / --backup" >&2; exit 1; }
...
if [ -n "$BRIEF" ]; then
  install_one daily-brief "$(calendar "${BRIEF%%:*}" "${BRIEF##*:}")" "$(prog "$WRAPPERS/run-daily-brief.sh")"
fi
if [ "$REMINDERS" -eq 1 ]; then
  install_one reminder-dispatch "  <key>StartInterval</key><integer>900</integer>" "$(prog "$WRAPPERS/run-reminder-dispatch.sh")"
fi
if [ -n "$BACKUP" ]; then
  # backup runs the repo script directly with the vault as its arg (no wrapper).
  install_one vault-backup-push "$(calendar "${BACKUP%%:*}" "${BACKUP##*:}")" "$(prog "$WRAPPERS/vault-backup-push.sh" "$VAULT_ABS")"
fi
